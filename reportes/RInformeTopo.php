...
class RInformeTopo extends Report
{
    ...
    function write($fileName)
    {
        ...
        // 1. Obtener el array del dataset usando el método getter de DataSource
        $dataset = $dataSource->getDataset();

        // 2. Acceder al objeto Mensaje con la clave 'ListarPersonas'
        $mensajePersonas = $dataset[0]['ListarPersonas'];

        // 3. Obtener el arreglo de personas (datos es una propiedad pública en la clase Mensaje)
        $personas = $mensajePersonas->datos;
        ...
        //var_dump($dataSource->getParameter('id_tipo_tramite')); exit();
        $firstBlock = 'Se procede a la inspección y verificación técnica del expediente del trámite administrativo N°  '.$dataSource->getParameter('cite_tramite');
        if ($dataSource->getParameter('id_tipo_tramite') == "3") {
            $firstBlock .= ', que pertenece a: <b>'. $propName.'</b>';
        }
        $firstBlock .= '<br>Con referencia a la aprobación de: <b>'. strtoupper($dataSource->getParameter('nombre_tramite')).'</b>';

        $pdf->writeHTMLCell(180, 0, '', '', $firstBlock , 0, 1, 0, true, 'J', true);
